...
export function UsoLimiteForm({ allocations }: { allocations: { id: string; label: string }[] }) {
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      <button type="submit" disabled={pending || allocations.length === 0} className="w-full rounded-lg bg-gold-600 px-4 py-2 text-sm font-medium text-black hover:bg-gold-500 disabled:opacity-50">
